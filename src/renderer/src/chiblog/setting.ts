interface chiblogSiteSetting {
  ...
  comment: {
    ...
    ghauth: {
      ...
      // Due to the security problem, the client secret is not supported on chiblog v1.1.3 and above.
  ...
  articleimage: {
    ...
    type: string // random: with random picture urls given by array below.; randomjson: with a navigtor json file
    images: string[] // article images
  }
  static: boolean // ignore mocks tip(Use only in a static site)
  desc: string // site description
}

export { type chiblogSiteSetting }
